module CouchRest
  class Server
    ...
    def initialize(server = nil, uuid_batch_count = nil)
      server = server.nil? ?
        CouchRest.configuration.server_url :
        server

      @uri = prepare_uri(server).freeze
      @uuid_batch_count = uuid_batch_count.nil? ?
        CouchRest.configuration.uuid_batch_count :
        uuid_batch_count
    end
